...
| Опция | Описание |
|-------|----------|
| `-d <домен>` | Быстрый тест домена без конфига (все проверки по умолчанию) |
...
./http-tester -d tavda.info -format json
...
| `https` | `true` | Проверять HTTPS на порту 443 (по умолчанию) |
| | `false` / отсутствие | HTTPS не проверяется |
| пустая секция `web:` | | `http: redirect`, `https: true` |
