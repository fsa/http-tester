...
|----------|----------|----------|
| `http` | `auto` | Порт 80: принимаем 200 или 301/302 (по умолчанию) |
| | `redirect` | Порт 80: ожидаем только 301/302 |
| | `direct` | Порт 80: ожидаем только 200 OK |
| | `no` / `false` | Порт 80 не проверяется |
| | `yes` / `true` | То же что `auto` |
| `https` | `true` / `yes` | Проверять HTTPS на порту 443 (по умолчанию) |
| | `false` / `no` | HTTPS не проверяется |
| пустая секция `web:` | | `http: auto`, `https: true` |
