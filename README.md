...
http: redirect  # HTTP проверки: redirect (по умолчанию) или direct
...
| Значение | Описание |
|----------|----------|
| `redirect` | Порт 80: ожидаем 301/302 редирект на HTTPS |
| `direct` | Порт 80: ожидаем 200 OK (сайт работает по HTTP) |
| `true` | То же что `redirect` (совместимость) |
| отсутствие | HTTP проверки отключены |

Автоматически выполняются:

1. **Порт 80** — HTTP/1.1 (в зависимости от режима)
